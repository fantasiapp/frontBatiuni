...
export class Notifications {
  ...
  get today(): NotificationDisplay[] {
    ...
    const today: NotificationDisplay[] = this.notificationsDisplay.filter(notif => moment(moment(todayDate).format('L')).isSame(moment(notif.date).format('L')))
    return today
  }

  get month(): NotificationDisplay[] {
    let todayDate = new Date(Date.now())
    const month: NotificationDisplay[] = this.notificationsDisplay.filter(notif => moment(moment(todayDate).format('L')).isAfter(moment(notif.date).format('L')))
    return month
  }
  ...
  get timerToday(): any[] {
    ...
    for (let i = 0; i < this.today.length; i++) {
      timerToday.push(moment(moment(this.today[i].date)).startOf('minute').fromNow());
    }
    return timerToday
  }
  ...
  get timerMonth(): any[] {
    let timerMonth = []
    for (let i = 0; i < this.today.length; i++) {
      timerMonth.push(moment(moment(this.timerMonth[i].date)).startOf('minute').fromNow());
    }
    return timerMonth
  }
      ...
      switch ( notification.nature ) {
        case "PME":
          if (notification.missions) {
            ...
            company = (mission) ? this.store.selectSnapshot(DataQueries.getById("Company", mission.company)) as Company  : null
          } else {
            let post = this.store.selectSnapshot(DataQueries.getById("Post", notification.posts)!) as Post;
            company = (post) ? this.store.selectSnapshot(DataQueries.getById("Company", post.company)) as Company: null
          }
          if (company) {
        ...
        case "ST":
          if (notification.missions) {
            let mission = this.store.selectSnapshot(DataQueries.getById("Mission", notification.missions)!) as Mission;
            company = (mission) ? this.store.selectSnapshot(DataQueries.getById("Company", mission.subContractor)) as Company : null
          } else {
            company = (notification.subContractor) ? this.store.selectSnapshot(DataQueries.getById("Company", notification.subContractor)!) as Company: null
          }
