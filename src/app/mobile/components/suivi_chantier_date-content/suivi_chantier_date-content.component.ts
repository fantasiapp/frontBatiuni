...
export class SuiviChantierDateContentComponent extends Destroy$ {
  ...
  validate(task: Task) {
    if (this.view == "ST" && !task.refused && !this.mission!.isClosed) {
      ...
      this.store
        .dispatch(new ModifyDetailedPost(task))
        .pipe(take(1))
        .subscribe(() => {
          let control = document.getElementById(
            "control_validate_" + task.id
          ) as HTMLImageElement;
  ...
  formTask = new FormGroup({
    commentTask: new FormControl("", [Validators.required]),
  });
