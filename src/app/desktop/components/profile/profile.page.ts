...
export class ProfilePageComponent {
  ...
  modifyProfileDesktop(binding: any) {
    ...
    const action = this.store.dispatch(
      new UserActions.ModifyUserProfile({
    ...
    action.pipe(take(1)).subscribe(
      (success) => {
        ...
        (form as FormGroup).markAsPristine();
        (form as FormGroup).markAsUntouched();
  ...
  changePasswordDesktop(form: FormGroup) {
    ...
    let req = this.store.dispatch(
      new UserActions.ChangePassword(oldPwd, newPwd)
    );
    req.subscribe(
      (success) => {
        this.info.show("success", "Mot de passe changé avec succès", 2000);
      },
      (error) => {
        this.info.show("error", "Erreur lors du changement du mot de passe");
      }
    );
  }
}
