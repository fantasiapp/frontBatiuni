...
import { GetGeneralData, HandleApplication, SignContract, MarkViewed, ModifyAvailability, SetFavorite, TakePicture, InviteFriend, ValidateMissionDate } from "./user/user.actions";
...
export class DataState {
    ...
    return this.http.post('data', application).pipe(
      tap((response:any) => {
        ...
        if ( response[application.action] !== 'OK' ) {
          ...
          this.inZone(() => this.info.show("info", 'La mission est mise à jour', 3000));
          delete response[application.action];
          ctx.setState(
            addComplexChildren('Company', profile.company.id, 'Mission', response)
          )
  ...
  notificationViewed(ctx:StateContext<DataModel>, application: NotificationViewed) {
    const profile = this.store.selectSnapshot(DataQueries.currentProfile)!;
    return this.http.post('data', application).pipe(
      tap((response:any) => {
        if ( response[application.action] !== 'OK' ) {
